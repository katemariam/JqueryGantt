...
<html lang="en-au">
        	...
        	<div class="col-md-5"><nr>
    			...
    			<form method='get' url='test_guil.php'>
    				<label style="position:relative; top:10px;" onchange="this.form.submit()"><b> Select Project</b> 
 				 	<select name="employee_name" id="employee_name">
						...
						<?php
							$sql = "SELECT employee_id ,employee_name FROM employee_tbl;";

							$query = $conn->query($sql);
							$output = "";
							while($row = mysqli_fetch_array($query)) {
								if($_GET["employee_name"] == $row["employee_id"]) {
					...
					<div style= "position: relative; top:-25px; left:380px;">  			
                		...
                		<label class="employee btn btn-sm btn-info active">
                    		<a href=" ">
				...
				<table class="table">
                	<thead>
                		...
                		<tbody id="task_list">
							<?php
								$emp_id = $_GET["employee_name"];
								if(!empty($_GET)) {
									$sql = "SELECT chart_task.id, chart_task.start, chart_task.end, chart_task.name  FROM emp_task INNER JOIN chart_task ON emp_task.chart_id = chart_task.id WHERE emp_task.emp_id = $emp_id;";
	...
	<script>
		...
		function generateGantt(){ 
			$.ajax({
				type: "GET",
				url: "fetchTasks(emp).php?emp_id= <?php echo $_GET['employee_name'] ?>" ,
				dataType: "json",
				success: (resp) => {
